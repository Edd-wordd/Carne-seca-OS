        ...
        <Dialog open={!!order} onOpenChange={(open) => !open && onClose()}>
            <DialogContent className="max-h-[min(90vh,720px)] overflow-y-auto border-zinc-800 bg-zinc-900 sm:max-w-lg">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        {order ? `Generate packing slip for ${order.id} — ${order.customer}` : ''}
                    </DialogDescription>
                </DialogHeader>
                {order ? (
                    <div className="rounded-lg border border-zinc-800 bg-zinc-950/50 p-4 text-sm">
                        <div className="flex items-start justify-between border-b border-zinc-800 pb-3">
                            ...
                            <div className="text-right text-xs text-zinc-400">
                                <p>
                                    Order ID: <span className="font-mono text-zinc-300">{order.id}</span>
                                </p>
                        ...
                        <div className="mt-3 space-y-1">
                            ...
                            <p className="text-zinc-100">{order.customer || '—'}</p>
                        </div>
                        <div className="mt-3 space-y-1">
                            <p className="text-[10px] uppercase tracking-wider text-zinc-500">Shipping address</p>
                            <p className="text-zinc-300 text-xs whitespace-pre-line">{formatAddress(order.address)}</p>
                        </div>
                        <div className="mt-4">
