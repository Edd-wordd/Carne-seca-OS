...
const REPORT_TYPES = [
    {
        ...
        icon: Wrench,
        description: 'Supply items, purchase history, and monthly spend',
    },
    {
        value: 'orders',
        label: 'Orders Report',
        icon: ShoppingBag,
        description: 'Orders, revenue, and fulfillment status',
    },
    {
        value: 'inventory',
        label: 'Inventory Report',
        icon: Box,
        description: 'Stock levels, value, and low-stock items',
    },
    ...
    const handleExportCsv = () => {
        ...
        if (reportType === 'supplies') {
            ...
            const supplyRows = MOCK_SUPPLIES.map((s) => {
                ...
                const qtyWeight =
                    s.weight != null ? `${s.quantity} ${s.unit} (${s.weight} lb)` : `${s.quantity} ${s.unit}`;
    ...
    const byMonth = React.useMemo(() => {
        ...
        return Object.entries(acc)
            ...
            .map(([month, total]) => {
                ...
                return {
                    month: new Date(parseInt(y), parseInt(mo) - 1).toLocaleDateString('en-US', {
                        month: 'short',
                        year: 'numeric',
                    }),
                    total,
    ...
    return (
        <div className="space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-4 print:hidden">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <Button
                        size="sm"
                        variant="outline"
                        className="gap-1.5 border-zinc-700 text-zinc-300 h-8 text-xs print:hidden"
                        onClick={handlePrint}
                    >
            ...
            {reportType === 'supplies' && (
                <div
                    ref={reportRef}
                    className="rounded border border-zinc-800 bg-zinc-900/60 p-6 text-zinc-100 print:border print:bg-white print:text-zinc-900"
                >
                    <div className="space-y-6">
                        <div>
                            <h2 className="text-lg font-semibold">Supplies Report</h2>
                            <p className="text-zinc-500 text-sm">
                                Carne Seca · Generated {new Date().toLocaleDateString('en-US', { dateStyle: 'long' })}
                            </p>
                        </div>
                        ...
                        <div>
                            ...
                            <p className="text-sm text-zinc-500 mt-1">
                                Monthly spend (last {byMonth.length} months):{' '}
                                {byMonth.map((m) => `${m.month}: $${m.total.toFixed(0)}`).join(', ')}
                            </p>
                        </div>
                        ...
                        <div>
                            ...
                            <table className="w-full text-sm border-collapse">
                                ...
                                <tbody>
                                    {MOCK_SUPPLIES.map((s) => {
                                        const cat =
                                            SUPPLY_CATEGORIES.find((c) => c.value === s.category)?.label ?? s.category;
                                        const qtyWeight =
                                            s.weight != null
                                        ...
                                        return (
                                            <tr
                                                key={s.id}
                                                className="border-b border-zinc-800/50 print:border-zinc-300"
                                            >
                            ...
                            <table className="w-full text-sm border-collapse">
                                ...
                                <tbody>
                                    {[...MOCK_PURCHASE_HISTORY]
                                        ...
                                        .map((h) => {
                                            const cat =
                                                SUPPLY_CATEGORIES.find((c) => c.value === h.category)?.label ??
                                                h.category;
                                            const pay =
                ...
                >
                    <div className="space-y-6">
                        ...
                        <div>
                            <h3 className="text-sm font-medium mb-2">Summary</h3>
                            <p className="text-sm">
                                Total revenue (non-refunded):{' '}
                                <strong>
                                    $
                                    {MOCK_ORDERS.filter((o) => o.status !== 'refunded')
                                        .reduce((s, o) => s + o.total, 0)
                                        .toLocaleString(undefined, { minimumFractionDigits: 2 })}
                                </strong>
                            </p>
                            <p className="text-sm text-zinc-500 mt-1">
                                Orders: {MOCK_ORDERS.length} · By status:{' '}
                                {[...new Set(MOCK_ORDERS.map((o) => o.status))].join(', ')}
                            </p>
                        </div>
                        <div>
                ...
                >
                    <div className="space-y-6">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-zinc-500 mt-1">
                                Products: {MOCK_INVENTORY.length} · Low stock: {lowStockCount} · Out of stock:{' '}
                                {MOCK_INVENTORY.filter((i) => i.stock === 0).length}
                            </p>
                        </div>
                        <div>
                            ...
                            <table className="w-full text-sm border-collapse">
                                ...
                                <tbody>
                                    {MOCK_INVENTORY.map((i) => {
                                        const status =
                                            i.stock === 0 ? 'Out of stock' : i.stock <= i.lowThreshold ? 'Low' : 'OK';
                                        return (
                                            <tr
                                                key={i.id}
                                                className="border-b border-zinc-800/50 print:border-zinc-300"
