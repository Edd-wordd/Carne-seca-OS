...
import { AreaChart, Area, Bar, BarChart, XAxis, YAxis, CartesianGrid } from 'recharts';
...
const OVERVIEW_ALERTS = [
    ...
    { id: 2, type: 'amber', title: 'Promoter debt', msg: '$1,240 unpaid — 3 partners pending payout', time: '1d ago' },
    {
        id: 3,
        type: 'info',
        title: 'Partner check-up',
        msg: 'Downtown Provisions — monthly check due Mar 1',
        time: '3d ago',
    },
    {
        id: 4,
        type: 'warning',
        title: 'Original 6oz out of stock',
        msg: 'Product hidden from shop until restocked',
        time: '5d ago',
    },
];
                    ...
                    <CardContent>
                        <ChartContainer config={chartConfig} className="h-[200px] w-full">
                            <BarChart data={SALES_VS_PRODUCTION} margin={{ top: 4, right: 4, bottom: 4, left: 0 }}>
                                ...
                                <ChartTooltip
                                    content={
                                        <ChartTooltipContent formatter={(v) => [`$${v?.toLocaleString() ?? v}`]} />
                                    }
                                />
                    ...
                    <ChartContainer config={conversionChartConfig} className="h-[180px] w-full">
                        <AreaChart data={CONVERSION_FUNNEL} margin={{ top: 4, right: 4, bottom: 4, left: 0 }}>
                            ...
                            <ChartTooltip
                                content={
                                    <ChartTooltipContent
                                        formatter={(v, name, item) => [`${v} (${item?.payload?.rate ?? ''} conv.)`]}
                                    />
                                }
                            />
                            <Area
                        ...
                        <TableBody>
                            {filteredProducts.map((p, i) => (
                                <TableRow key={p.rank} className="border-zinc-800/50">
                                    <TableCell className="text-zinc-500 px-3 py-1.5 font-mono text-[10px]">
                                        {i + 1}
                                    </TableCell>
                                    <TableCell className="text-zinc-200 px-3 py-1.5 text-[11px] font-medium">
                                        {p.name}
                                    </TableCell>
                                    <TableCell className="text-zinc-400 px-3 py-1.5 text-right tabular-nums text-[11px]">
                                        {p.units}
                                    </TableCell>
                                    <TableCell className="text-emerald-400 px-3 py-1.5 text-right tabular-nums text-[11px] font-medium">
